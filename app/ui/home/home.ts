...
interface CreateCars {
  ...
  cPos: number
  cColor?: string
  createCarPos: { value: boolean }
  city: THREE.Object3D
}

function createCars(props: CreateCars) {
  const { cScale = 2, cPos = 20, cColor = 0xffff00, createCarPos, city } = props
  ...
  if (createCarPos.value) {
    createCarPos.value = false
    cElem.position.x = -cPos
    cElem.position.z = mathRandom(cAmp)

    gsap.to(cElem.position, 3, { x: cPos, repeat: -1, yoyo: true, delay: mathRandom(3) })
  } else {
    createCarPos.value = true
    cElem.position.x = mathRandom(cAmp)
    cElem.position.z = -cPos
    cElem.rotation.y = (90 * Math.PI) / 180

    gsap.to(cElem.position, 5, { z: cPos, repeat: -1, yoyo: true, delay: mathRandom(3) })
  }
...
function animate(props: Animate) {
  const { city, town, smoke, uSpeed, camera, renderer, scene } = props
...
export function initHomePage() {
  ...
  // Three JS Template
  //----------------------------------------------------------------- BASIC parameters
  const renderer = new THREE.WebGLRenderer({ antialias: true, canvas: document.getElementById('galaxy') as any })
  ...
  for (var i = 0; i < 60; i++) {
    createCars({ cScale: 0.1, cPos: 20, createCarPos, city })
  }

  init({ smoke, city, town })

  animate({ city, town, smoke, uSpeed, camera, renderer, scene })
